...
const page = () => {
    ...
    const verifyAccount = async(e) => {
      ...
      try {
        ...
        if(!isSentCodeEmail) return setVerifyError('أرسل رمز للتحقق من ملكيتك للبريد الالكتروني.');
    ...
    if(userId.length <= 10 || isVerified){
        return (fetchingUserInfo && !isVerified) ? <MySkeleton isMobileHeader/> : <NotFound type={'not allowed'}/>
    }
    ...
    <div className='SignUp' style={{ minHeight: 'unset' }}>
      ...
      <div className='formDiv'>
        ...
        <form onSubmit={verifyAccount}>
            ...
            <button type='button' className='btnbackscndclr' style={{ margin: '-32px 0 24px 0' }} onClick={sendCodeToEmail}>{sendingCode ? <LoadingCircle /> : 'ارسال رمز تحقق الى بريدك الالكتروني'}</button>
            ...
            <CustomInputDiv title={'ادخل الرمز'}  isError={code === '-1'} errorText={'الرجاء ادخال رمز تحقق صالح.'} listener={(e) => setCode(e.target.value)}/>

            <label id='success' style={!operationSuccess ? { marginTop: -44, padding: 0 } : { marginTop: -24 }}>{operationSuccess && 'تم تفعيل بنجاح'} <Link href={`/profile?id=${userId}`} style={{ display: !operationSuccess && 'none' }}>الذهاب الى الملف الشخصي</Link></label>

            <button type='submit' className='btnbackscndclr' onClick={verifyAccount} style={{ marginTop: 0 }}>{verifing ? <LoadingCircle /> : 'تفعيل الحساب'}</button>
