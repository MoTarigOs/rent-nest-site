...
const page = () => {
    ...
    const verifyAccount = async(e) => {
      ...
      try {
        ...
        if(!isSentCodeEmail) return setVerifyError('Send a verification code to your Email.');
        ...
        setIsVerified(true);
        setVerifing(false);
        location.href = `/en/profile?id=${userId}`;
    ...
    if(userId.length <= 10 || isVerified){
        return (fetchingUserInfo && !isVerified) ? <MySkeleton isMobileHeader/> : <NotFound isEnglish type={'not allowed'}/>
    }
    ...
    <div className='SignUp' style={{ minHeight: 'unset' }} dir='ltr'>
      ...
      <div className='formDiv'>
        ...
        <form onSubmit={verifyAccount}>
            ...
            <button type='button' className='btnbackscndclr' style={{ margin: '-32px 0 24px 0' }} onClick={sendCodeToEmail}>{sendingCode ? <LoadingCircle /> : 'Send verification code to your Email'}</button>
            ...
            <button type='submit' className='btnbackscndclr' onClick={verifyAccount} style={{ marginTop: 0 }}>{verifing ? <LoadingCircle /> : 'Activate Account'}</button>
